...
pub fn send_invitation(verification: &Veri) -> Result<(), ServiceError> {
    ...
    // recipient from the verification email
    let recipient: Recipient = verification.email.as_str().into();

    let email_body = format!(
        "Please click on the link below to complete registration. <br/>
         <a href=\"http://localhost:8080/register.html?id={}&email={}\">
         http://localhost:8080/register</a> <br>
         your verification expires on <strong>{}</strong>",
        verification.id,
        verification.email,
        verification
            .expires_at
